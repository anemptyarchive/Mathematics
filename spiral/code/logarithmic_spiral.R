...
# 角度線の座標を作成
angle_oblique_df <- tibble::tibble(
  frame_i = 1:frame_num, 
  t = t_vals, 
  r = a * exp(b * t), 
  x = axis_size * cos(t), 
...
for(i in 1:frame_num) {
  ...
  ggplot2::ggsave(filename = file_path, plot = graph, width = 1500, height = 1200, units = "px", dpi = 100)
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  ...
  magick::image_write_gif(path = "spiral/figure/logarithmic/curves_variable.gif", delay = 1/30) -> tmp_path # gifファイル書き出し
...
# グラフサイズを設定
axis_size <- c(
  max(a_vals)*exp(max(b_vals) * t_vec) * cos(t_vec), 
  max(a_vals)*exp(max(b_vals) * t_vec) * sin(t_vec)
) |> 
  abs() |> 
  max() |> 
  ceiling()
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  magick::image_read() |> # 画像ファイルを読み込み
  magick::image_animate(fps = 1, dispose = "previous") |> # gif画像を作成
  magick::image_write_gif(path = "spiral/figure/logarithmic/curves_param.gif", delay = 1/10) -> tmp_path # gifファイル書き出し
