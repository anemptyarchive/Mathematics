...
ggplot() + 
  ...
  geom_line(data = curve_df,
            mapping = aes(x = theta, y = cot_t, linetype = inv_flag, color = "cot"), 
...
ggplot() + 
  geom_line(data = curve_inv_df, 
            ...
            linewidth = 1) + # 逆関数
  geom_line(data = curve_df,
            mapping = aes(x = theta, y = cot_t, color = "cot"), 
  ...
  labs(title = "inverse cotangent function", 
       subtitle = parse(text = def_label), 
       x = expression(theta == arccot~x), 
       y = expression(x == cot~theta))
...
rad_tick_df <- tibble::tibble(
  ...
  grid      = dplyr::if_else(major_flag, true = "major", false = "minor") # 線幅用
)
...
for(i in 1:frame_num) {
  ...
  radius_df <- tibble::tibble(
    ...
    w         = c("normal", "normal", "thin", "thin", "thin"), # 線幅用
    line_type = c("main", "main", "main", "main", "sub")  # 反転の描き分け用
  )
  ...
  ggplot2::ggsave(filename = file_path, plot = wrap_graph, width = 1400, height = 800, units = "px", dpi = 100)
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  ...
  magick::image_write_gif(path = "circular/figure/inverse/arccot_curves_variable.gif", delay = 1/15) -> tmp_path # gifファイルを書出
