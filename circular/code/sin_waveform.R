...
ggplot() + 
  geom_line(data = curve_df, 
            ...
            linewidth = 1) + # 元の曲線
  geom_line(data = curve_df, 
            mapping = aes(x = theta, y = f_t, linetype = "f t"), 
            linewidth = 1) + # 変形した曲線
  scale_x_continuous(breaks = rad_break_vec, 
                     labels = parse(text = rad_label_vec)) + # ラジアン軸目盛
  scale_linetype_manual(breaks = c("f t", "sin t"), 
...
for(i in 1:frame_num) {
  ...
  radius_df <- dplyr::bind_rows(
    ...
    fnc_point_df |> 
      ...
      dplyr::mutate(
        line_type = dplyr::if_else(fnc == "sin t", true = "main", false = "rev")
      )
  )
  ...
  circle_graph <- ggplot() + 
    ...
    geom_segment(data = radius_df, 
                 mapping = aes(x = 0, y = y0, xend = x, yend = y, linetype = line_type), 
    ...
    geom_point(data = fnc_point_df, 
               ...
               size = 4) + # 関数点
    scale_color_manual(breaks = c("sin t", "f t", "a sin at", "alpha"), 
                       values = c("blue", "red", "orange", "purple"), 
                       labels = parse(text = c("sin~theta", "A ~ sin(a * theta + alpha)", "A ~ sin(a * theta)", "alpha")), 
  ...
  curve_graph <- ggplot() + 
    ...
    scale_x_continuous(breaks = rad_break_vec, 
                       labels = parse(text = rad_label_vec)) + # ラジアン軸目盛
    scale_color_manual(breaks = c("sin t", "f t", "a sin at"), 
                       values = c("blue", "red", "orange"), 
                       labels = parse(text = c("sin~theta", "A ~ sin(a * theta + alpha) + C", "A ~ sin(a * theta) + C")), 
                       name = "function") + # 関数ごとに色分け
    scale_linetype_manual(breaks = c("sin t", "f t", "a sin at"), 
