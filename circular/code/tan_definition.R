...
anim_label_df <- tibble::tibble(
  ...
  var_label = paste0(
    ...
    "tan~theta == ", round(tan(t), digits = 2), 
    ")"
  )
)
fnc_label_vec <- c(tan = "tangent", sin = "sine", cos = "cosine", exsec = "exsecant")
...
gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "circular/figure/tangent/definition_circle_1.mp4")
)
...
# 動画を作成
gganimate::animate(
  plot = anim, nframes = frame_num, fps = 15, width = 8, height = 7, units = "in", res = 250, 
  renderer = gganimate::av_renderer(file = "circular/figure/tangent/definition_circle_2.mp4")
)
...
for(i in 1:frame_num) {
  ...
  fnc_label_vec <- paste(
    ...
    c(tan(theta), sin(theta), cos(theta), 1/cos(theta-1)) |> 
      round(digits = 2), 
    sep = " == "
  )
  ...
  circle_graph <- ggplot() + 
    ...
    geom_text(data = angle_label_df, 
              ...
              label = "theta", parse = TRUE, 
              size = 5) + # 角ラベル
    geom_segment(mapping = aes(x = 1, y = tan(theta), 
                               xend = Inf, yend = tan(theta)), 
  ...
  curve_graph <- ggplot() + 
    ...
    geom_line(data = fnc_curve_df, 
              ...
              linewidth = 1) + # 関数曲線
    geom_segment(mapping = aes(x = theta, y = tan(theta), 
                               xend = c(-Inf, theta), yend = c(tan(theta), -Inf)), 
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  ...
  magick::image_write_video(path = "circular/figure/tangent/definition_curves_coord.mp4", framerate = 30) -> tmp_path # mp4ファイルを書出
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  magick::image_read() |> # 画像ファイルを読込
  magick::image_animate(fps = 1, dispose = "previous") |> # gif画像を作成
  magick::image_write_video(path = "circular/figure/tangent/definition_curves_form.mp4", framerate = 30) -> tmp_path # mp4ファイルを書出
