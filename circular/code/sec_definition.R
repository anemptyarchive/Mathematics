...
anim_label_df <- tibble::tibble(
  ...
  var_label = paste0(
    ...
    "sec~theta == ", round(1/cos(t), digits = 2), 
    ")"
  )
)
fnc_label_vec <- c(sec = "secant", sin = "sine", cos = "cosine", tan = "tangent")
...
gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "circular/figure/secant/definition_circle_1.mp4")
)
...
anim_fnc_seg_df <- tibble::tibble(
  ...
  x_from = c(
    ...
    cos(theta_vals), abs(cos(theta_vals))*cos(theta_vals), 
    rep(0, times = frame_num), rep(0, times = frame_num), 
    cos(theta_vals)
  ), 
  y_from = c(
    rep(0, times = frame_num), 
    rep(0, times = frame_num), abs(cos(theta_vals))*sin(theta_vals), 
    rep(0, times = frame_num), rep(0, times = frame_num), 
    sin(theta_vals)
  ), 
  x_to = c(
    1/cos(theta_vals), 
    cos(theta_vals), ifelse(cos(theta_vals) >= 0, yes = 1, no = -1), 
    cos(theta_vals), abs(cos(theta_vals))*cos(theta_vals), 
    1/cos(theta_vals)
  ), 
  y_to = c(
...
gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "circular/figure/secant/definition_circle_2.mp4")
)
...
for(i in 1:frame_num) {
  ...
  circle_graph <- ggplot() + 
    ...
    geom_text(data = angle_label_df, 
              ...
              label = "theta", parse = TRUE, 
              size = 5) + # 角ラベル
    geom_segment(mapping = aes(x = 0, y = 1/cos(theta), xend = Inf, yend = 1/cos(theta)), 
  ...
  coord_label <- paste0(
    "(list(theta, f(theta))) == ", 
    "(list(", round(theta, digits = 2), ", ", round(1/cos(theta), digits = 2), "))"
  )
  ...
  curve_graph <- ggplot() + 
    ...
    geom_line(data = fnc_curve_df, 
              ...
              linewidth = 1) + # 関数曲線
    geom_segment(mapping = aes(x = theta, y = 1/cos(theta), 
                               xend = c(-Inf, theta), yend = c(1/cos(theta), -Inf)), 
  ...
  ggplot2::ggsave(filename = file_path, plot = wrap_graph, width = 12, height = 10, units = "in", dpi = 100)
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  ...
  magick::image_write_video(path = "circular/figure/secant/definition_curves_coord.mp4", framerate = 30) -> tmp_path # mp4ファイルを書出
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  magick::image_read() |> # 画像ファイルを読込
  magick::image_animate(fps = 1, dispose = "previous") |> # gif画像を作成
  magick::image_write_video(path = "circular/figure/secant/definition_curves_form.mp4", framerate = 30) -> tmp_path # mp4ファイルを書出
...
for(i in 1:frame_num) {
  ...
  curve_cos_graph <- ggplot() + 
    ...
    geom_segment(data = fnc_point_df, 
                 ...
                 color = color_vec[3], linewidth = 1) + # 関数線分
    scale_x_continuous(breaks = rad_break_vec, 
                       labels = parse(text = rad_label_vec)) + # θ軸目盛
    coord_fixed(ratio = 1, 
                ylim = c(-axis_y_size, axis_y_size)) + 
  ...
  # 画像ファイルを書出
  file_path <- paste0(dir_path, "/", stringr::str_pad(i, width = nchar(frame_num), pad = "0"), ".png")
  ggplot2::ggsave(filename = file_path, plot = wrap_graph, width = 10, height = 12, units = "in", dpi = 100)
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  magick::image_read() |> # 画像ファイルを読込
  magick::image_animate(fps = 1, dispose = "previous") |> # gif画像を作成
  magick::image_write_video(path = "circular/figure/secant/definition_curves_cos_coord.mp4", framerate = 30) -> tmp_path # mp4ファイルを書出
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  magick::image_read() |> # 画像ファイルを読込
  magick::image_animate(fps = 1, dispose = "previous") |> # gif画像を作成
  magick::image_write_video(path = "circular/figure/secant/definition_curves_cos_form.mp4", framerate = 30) -> tmp_path # mp4ファイルを書出
