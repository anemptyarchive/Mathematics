...
anim_asymptote_df <- tibble::tibble(
  frame_i = 1:frame_num, 
  abs_a   = abs(a_vals) + 1e-10, # (ゼロ除算の回避用の微小値)
  ...
  dplyr::mutate(
    t = dplyr::if_else(
      (t >= min(theta_vec) & t <= max(theta_vec)), true = t, false = NA_real_
    )
  ) # 描画範囲外の値を欠損値に置換
...
anim_label_df <- tibble::tibble(
  ...
  a       = a_vals, 
  x_from  = 0, 
  x_to    = pi / (abs(a)+ 1e-10), # (ゼロ除算の回避用の微小値)
  x_med   = 0.5 * x_to, 
  y       = 0, 
...
anim <- ggplot() + 
  ...
  geom_segment(data = anim_label_df,
               mapping = aes(x = 0, y = y, xend = pi, yend = y),
  ...
  geom_text(data = anim_label_df,
            mapping = aes(x = 0.5*pi, y = y),
            label = "pi", parse = TRUE, color = "blue", vjust = 1.5, size = 6) + # 元の範囲ラベル
  geom_text(data = anim_label_df,
            mapping = aes(x = x_med, y = y),
            label = "frac(pi, '|'*a*'|')", parse = TRUE, color = "red", vjust = -0.2, size = 6) + # 変形した範囲ラベル
  geom_text(data = anim_label_df, 
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  ...
  magick::image_write_gif(path = "circular/figure/tangent/period_curves_variable.gif", delay = 1/20) -> tmp_path # gifファイルを書出
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  magick::image_read() |> # 画像ファイルを読込
  magick::image_animate(fps = 1, dispose = "previous") |> # gif画像を作成
  magick::image_write_gif(path = "circular/figure/tangent/period_curves_param.gif", delay = 1/20) -> tmp_path # gifファイルを書出
