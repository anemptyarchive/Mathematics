...
anim_fnc_label_df <- dplyr::bind_rows(
  ...
  # 逆円関数ラベル
  tibble::tibble(
    fnc = factor("arctan", levels = fnc_level_vec), 
    frame_i = 1:frame_num, 
    x = cos(0.5 * tau_vals), 
    y = sin(0.5 * tau_vals), 
    fnc_label = "arctan(tan~theta)", 
    a = 0, 
    h = -0.1, 
    v = 0.5
  )
...
fnc_label_vec <- c(
  arctan = "inverse tangent", tan = "tangent", sin = "sine", cos = "cosine", exsec = "exsecant"
)
...
anim <- ggplot() + 
  ...
  geom_path(data = anim_radian_df, 
            mapping = aes(x = x, y = y, color = "arctan"), 
            linewidth = 1) + # 関数円弧
...
gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "circular/figure/inverse/arctan_circle.mp4")
)
...
for(i in 1:frame_num) {
  ...
  fnc_seg_df <- tibble::tibble(
    ...
    w = c("bold", "thin", "normal", "normal", "normal"), # 重なり対策用
    line_type = c("sub", "main", "main", "main", "main") # 補助線用
  )
  ...
  curve_graph <- ggplot() + 
    ...
    labs(title = "inverse tangent function", 
         ...
         x = expression(z == tan~tau == tan~theta), 
  ...
  ggplot2::ggsave(filename = file_path, plot = wrap_graph, width = 15, height = 10, units = "in", dpi = 120)
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  ...
  magick::image_write_video(path = "circular/figure/inverse/arctan_curves.mp4", framerate = 15) -> tmp_path # mp4ファイルを書出
