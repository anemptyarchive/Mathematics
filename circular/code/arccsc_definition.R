...
# 関数の描画順を指定
fnc_level_vec <- c("arccsc", "csc", "sin", "cos", "cot")
...
anim_fnc_label_df <- dplyr::bind_rows(
  ...
  # 逆円関数ラベル
  tibble::tibble(
    fnc = factor("arccsc", levels = fnc_level_vec), 
    frame_i = 1:frame_num, 
    x = cos(0.5 * tau_vals), 
    y = sin(0.5 * tau_vals), 
    fnc_label = "arccsc(csc~theta)", 
    a = 0, 
    h = -0.1, 
    v = 0.5
  )
...
fnc_label_vec <- c(
  arccsc = "inverse cosecant", csc = "cosecant", sin = "sine", cos = "cosine", cot = "cotangent"
)
...
anim <- ggplot() + 
  ...
  geom_path(data = anim_radian_df, 
            mapping = aes(x = x, y = y, color = "arccsc"), 
            linewidth = 1) + # 関数円弧
  ...
  scale_linewidth_manual(breaks = c("bold", "normal", "thin", "main", "sub", "major", "minor"), 
                         values = c(1.5, 1, 0.5, 1, 0.5, 0.5, 0.25)) + # 補助線用, 主・補助目盛線用
...
gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "circular/figure/inverse/arccsc_circle.mp4")
)
...
for(i in 1:frame_num) {
  ...
  fnc_label_df <- tibble::tibble(
    ...
    x = c(
      1, 
      0.5 * ifelse(test = rev_flag, yes = -1/tan(theta), no = 1/tan(theta))
    ), 
    y = c(
      0.5 * tau, 
      0.5
    ), 
    fnc_label = c("arccsc~z", "csc~theta"), 
    a = c(90, 0), 
    h = c(0.5, 1.1), 
    v = c(1.5, 0.5)
  )
  ...
  fnc_label_vec <- paste(
    c("arccsc~z", "csc~theta", "sin~theta", "cos~theta", "cot~theta"), 
    c(tau, 1/sin(theta), sin(theta), cos(theta), 1/tan(theta)) |> 
      round(digits = 2), 
    sep = " == "
  )
  ...
  coord_label <- paste0(
    "(list(z, tau)) == ", 
    "(list(", round(1/sin(theta), digits = 2), ", ", round(tau, digits = 2), "))"
  )
  ...
  curve_graph <- ggplot() + 
    ...
    geom_line(data = curve_df, 
              mapping = aes(x = z, y = arccsc_z), 
              linewidth = 1) + # 関数曲線
    geom_point(data = fnc_point_df, 
               mapping = aes(x = csc_t, y = arccsc_z), 
               size = 4) + # 関数点
    geom_segment(mapping = aes(x = c(1/sin(theta), 1/sin(theta)), y = c(tau, tau), 
                               xend = c(-Inf, 1/sin(theta)), yend = c(tau, -Inf)), 
                 linewidth = 0.8, linetype = "dotted") + # z・τ軸の目盛線
    geom_segment(data = fnc_seg_df, 
                 mapping = aes(x = z_from, y = y_from, xend = z_to, yend = y_to, color = fnc), 
                 linewidth = 1) + # 関数線分
    scale_color_manual(breaks = c("arccsc", "csc"), 
    ...
    coord_fixed(ratio = 1, 
                ...
                ylim = c(-axis_y_size, axis_y_size)) + 
    labs(title = "inverse cosecant function", 
         subtitle = parse(text = coord_label), 
         x = expression(z == csc~tau == csc~theta), 
         y = expression(tau == arccsc~z))
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  ...
  magick::image_write_video(path = "circular/figure/inverse/arccsc_curves.mp4", framerate = 30) -> tmp_path # mp4ファイルを書出
