...
anim_fnc_label_df <- dplyr::bind_rows(
  ...
  # 逆円関数ラベル
  tibble::tibble(
    fnc = factor("arccos", levels = fnc_level_vec), 
    frame_i = 1:frame_num, 
    x = cos(0.5 * tau_vals), 
    y = sin(0.5 * tau_vals), 
    fnc_label = "arccos(cos~theta)", 
    a = 0, 
    h = -0.1, 
    v = -0.5
  )
...
fnc_label_vec <- c(arccos = "inverse cosine", cos = "cosine", sin = "sine")
...
anim <- ggplot() + 
  ...
  geom_path(data = anim_radian_df, 
            mapping = aes(x = x, y = y, color = "arccos"), 
            linewidth = 1) + # 関数円弧
...
# gifファイルを書き出し
gganimate::anim_save(animation = gif, path ="circular/figure/inverse",  filename = "arccos_circle.gif")
...
for(i in 1:frame_num) {
  ...
  curve_graph <- ggplot() + 
    ...
    geom_segment(mapping = aes(x = c(cos(theta), cos(theta)), y = c(tau, tau), 
                 ...
                 color = c("red", "black"), linewidth = 0.8, linetype = "dotted") + # x・τ軸線の目盛線
    geom_segment(data = fnc_seg_df, 
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  ...
  magick::image_write_gif(path = "circular/figure/inverse/arccos_curves_variable.gif", delay = 1/25) -> tmp_path # gifファイルを書出
