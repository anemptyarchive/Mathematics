...
curve_df <- tibble::tibble(
  ...
  cos_t = cos(theta), 
  inv_flag = (theta >= 0 & theta <= pi) # 逆関数の値域
)
...
ggplot() + 
  geom_line(data = curve_inv_df, 
            mapping = aes(x = x, y = arccos_x, color = "arccos"), 
            linewidth = 1) + # 逆関数
  geom_line(data = curve_df,
            mapping = aes(x = theta, y = cos_t, linetype = inv_flag, color = "cos"), 
            linewidth = 1) + # 元の関数
  geom_abline(slope = 1, intercept = 0, 
              linetype = "dotdash") + # 恒等関数
  scale_color_manual(breaks = c("arccos", "cos"), 
                     values = c("black", "orange"), 
                     labels = c(expression(arccos~x), expression(cos~x)), 
                     name = "function") + # 凡例表示用
  scale_linetype_manual(breaks = c(TRUE, FALSE), 
                        values = c("solid", "dotted"), 
                        guide ="none") + 
  coord_fixed(ratio = 1) + # アスペクト比
  labs(title = "inverse cosine function", 
...
ggplot() + 
  geom_line(data = curve_inv_df, 
            mapping = aes(x = arccos_x, y = x, color = "arccos"), 
            linewidth = 1) + # 逆関数
  geom_line(data = curve_df,
            mapping = aes(x = theta, y = cos_t, color = "cos"), 
            linewidth = 1, linetype = "dashed") + # 元の関数
  scale_color_manual(breaks = c("arccos", "cos"), 
                     values = c("black", "orange"), 
                     labels = c(expression(arccos~x), expression(cos~theta)), 
                     name = "function") + # 凡例表示用
  ...
  labs(title = "inverse cosine function", 
       subtitle = parse(text = def_label), 
       x = expression(theta == arccos~x), 
       y = expression(x == cos~theta))


# 関数曲線を作図
ggplot() + 
  geom_line(data = curve_inv_df, 
            mapping = aes(x = x, y = arccos_x, linetype = "arccos"), 
            linewidth = 1) + # arccos関数
  geom_line(data = curve_inv_df,
            mapping = aes(x = x, y = arcsin_x, linetype = "arcsin"), 
            linewidth = 1) + # arcsin関数
  scale_linetype_manual(breaks = c("arccos", "arcsin"), 
                        values = c("solid", "dotted"), 
                        labels = c(expression(arccos~x), expression(arcsin~x)), 
                        name = "function") + # 凡例表示用
...
rad_tick_df <- tibble::tibble(
  ...
  grid       = dplyr::if_else(major_flag, true = "major", false = "minor")
)
...
# 関数の描画順を指定
fnc_level_vec <- c("arccos", "cos", "sin")
...
for(i in 1:frame_num) {
  ...
  fnc_label_df <- tibble::tibble(
    ...
    fnc_label = c("arccos~z", "cos~theta"), 
    a = c(90, 0), 
    h = c(0.5, 0.5), 
    v = c(1.5, 1)
  )
  ...
  fnc_label_vec <- paste(
    c("arccos~z", "cos~theta", "sin~theta"), 
    c(tau, cos(theta), sin(theta)) |> 
      round(digits = 2), 
    sep = " == "
  )
  ...
  coord_label <- paste0(
    "(list(z, tau)) == ", 
    "(list(", round(cos(theta), digits = 2), ", ", round(tau, digits = 2), "))"
  )
  ...
  curve_graph <- ggplot() + 
    geom_segment(mapping = aes(x = c(-Inf, 0), y = c(0, -Inf), 
                               xend = c(Inf, 0), yend = c(0, Inf)),
                 arrow = arrow(length = unit(10, units = "pt"), ends = "last")) + # x・τ軸線
    geom_line(data = curve_inv_df, 
              mapping = aes(x = z, y = arccos_z), 
              linewidth = 1) + # 関数曲線
    geom_point(data = fnc_point_df, 
               mapping = aes(x = cos_t, y = arccos_z), 
               size = 4) + # 関数点
    geom_segment(mapping = aes(x = c(cos(theta), cos(theta)), y = c(tau, tau), 
                               xend = c(-Inf, cos(theta)), yend = c(tau, -Inf)), 
                 color = c("#F8766D", "black"), linetype = "dotted") + # x・τ軸線の補助線
    geom_segment(data = fnc_seg_df, 
    ...
    labs(title = "inverse cosine function", 
         ...
         x = expression(z == cos~theta), 
         y = expression(tau == arccos~z))
  ...
  ggplot2::ggsave(filename = file_path, plot = wrap_graph, width = 900, height = 700, units = "px", dpi = 100)
...
paste0(dir_path, "/", stringr::str_pad(1:frame_num, width = nchar(frame_num), pad = "0"), ".png") |> # ファイルパスを作成
  ...
  magick::image_write_gif(path = "circular/figure/inverse/arccos_curves_variable.gif", delay = 1/15) -> tmp_path # gifファイルを書出
