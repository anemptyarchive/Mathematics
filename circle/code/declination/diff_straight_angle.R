...
angle_label_vec <- c("theta", "-theta", "tau == pi - theta")
...
anim <- ggplot() + 
  ...
  geom_text(data = rad_tick_df, 
            ...
            parse = TRUE) + # θ軸目盛ラベル
  geom_vline(xintercept = 0, linetype = "dashed") + # 対象の軸
...
gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "circle/figure/declination/diff_straight_angle_symmetry.mp4")
)
