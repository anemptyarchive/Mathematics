...
anim_fnc_seg_df <- tibble::tibble(
  ...
  x_from = c(
    ...
    cos(tau_vals), rep(0, times = frame_num)
  ), 
  y_from = c(
  ...
  y_to = c(
    ...
    sin(tau_vals), rep(0, times = frame_num)
  ), 
  label_flag = c(
...
angle_label_vec <- c("theta", "tau == -theta")
...
anim <- ggplot() + 
  ...
  geom_text(data = rad_tick_df, 
            ...
            parse = TRUE) + # θ軸目盛ラベル
  # geom_hline(yintercept = 0, linetype = "dashed") + # 対称の軸
  # geom_line(data = anim_point_df,
  #           mapping = aes(x = x, y = y),
  ...
  scale_shape_manual(breaks = c("main", "target"), 
                     values = c("circle", "circle open")) + # 補助点用
  scale_linetype_manual(breaks = c("main", "target"), 
...
gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "circle/figure/declination/neg_angle.mp4")
)
