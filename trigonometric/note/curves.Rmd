...
function_line_df <- tibble::tibble(
  ...
  type = c(
    "main", ifelse(test = sin_flg, yes = NA, no = "sub"), 
    "main", 
    "main", 
    "main", "sub"
  ) # 線タイプ用
...
anim_function_line_df <- tibble::tibble(
  ...
  type = c(
    rep("main", times = frame_num), ifelse(test = sin_flg_i, yes = NA, no = "sub"), 
...
　垂直線になるようにx軸の値を回転する曲線を描画するためのデータフレームを作成します。
...
　任意の範囲で設定したラジアン $\theta$ を $0 \leq \alpha \leq \pi$ の範囲のラジアンに変換します。コサイン関数 $\cos x$ の逆関数(逆コサイン関数) $\arccos x$ を使って、 $\alpha = \arccos(\cos \theta)$ で計算します(他の三角関数でも求められます)。逆コサイン関数は`acos()`で計算できます。\
...
　1つ目の方法では、sec関数とcos関数を並べて可視化します。\
\
