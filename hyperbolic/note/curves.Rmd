...
for(i in 1:frame_num) {
  ...
  cosh_graph <- ggplot() + 
    ...
    geom_segment(data = segment_cosh_df, 
                 ...
                 linetype = "dotted") + # 双曲線との対応線
    geom_point(data = point_df, 
               mapping = aes(x = t, y = cosh_t), size = 4) + # tanh曲線上の点
    coord_cartesian(clip = "off", 
                    xlim = c(min(theta_vec), max(theta_vec)), ylim = c(-axis_max, axis_max)) + # 
    labs(title = "cosh function", 
         subtitle = parse(text = cosh_label), 
...
　$\sinh x$はハイパボリックサイン関数、$\cosh x$はハイパボリックコサイン関数です。また、$e^x$はネイピア数$e$を底とする自然指数関数です。\
...
　$\sinh x$はハイパボリックサイン関数です。また、$e^x$はネイピア数$e$を底とする自然指数関数です。\
...
　変数が$\theta = 0$のとき$\sinh \theta = 0$なので、$\csc \theta = \frac{1}{\sinh \theta}$は0除算になるため定義されません。\
\
...
　csch関数の値は、「原点$(0, 0)$と点$(\cosh \theta, 1)$を通る直線」と「$y = 1$の直線」の交点のx軸の値に対応します。\
\
...
　$\cosh x$はハイパボリックコサイン関数です。また、$e^x$はネイピア数$e$を底とする自然指数関数です。\
\
...
　tanh関数はハイパボリックタンジェント関数、$\sinh x$はハイパボリックサイン関数、$\cosh x$はハイパボリックコサイン関数です。また、$e^x$はネイピア数$e$を底とする自然指数関数です。\
\
